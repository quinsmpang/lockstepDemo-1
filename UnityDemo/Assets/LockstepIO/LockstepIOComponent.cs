...
public class LockstepIOComponent : MonoBehaviour 
{
	...
	float IssueTimeTick;
	public float IssueTimeGap = 0.15f;
	public void Update ()
	{
...
//				IssueCommand(j);
//			}
			if (IssueTimeTick < Time.time)
			{
				IssueTimeTick = Time.time + IssueTimeGap;
			}
			JSONObject j = new JSONObject();
	...
	public void ExecuteCommand(JSONObject Command)
	{
		...
		Rigidbody testRigidBody = GameObject.Find("TestObject").GetComponent<Rigidbody>();
		float speed = 100;

		if (Command.HasField("x"))
		{
			testRigidBody.AddForce(new Vector2((float)Command.GetField("x").n * speed, 0));
		}
		if (Command.HasField("y"))
		{
			testRigidBody.AddForce(new Vector2(0, (float)Command.GetField("y").n * speed));
		}
	}
